...
<body>
<div class="loc">
  <div class="icon">当前位置：陈列手册列表</div>
</div>
<span class="con2">
<input name="button2" id="button2" value="添加陈列手册" class="blue" onclick="location.href='toEdit'" type="button">
<br>
<br>
</span>
  ...
  <#if list??>
	<#list list as s>
      <tr onMouseOut="this.style.backgroundColor='ffffff'" onMouseOver="this.style.backgroundColor='FFECA2'" class="tr_display"> 
	      ...
	      <td bgcolor="#FFFFFF">${s.parent.name?if_exists}</td>
...
function deleteOne(id){
	...
	$.ajax({
		url: '${base_addr}/gtb/displayHandbook/delete' ,
        secureuri: false,
        data: {id: id},
        dataType: 'text',
