...
<body>
<div class="loc">
    <div class="icon">当前位置：柜台任务列表</div>
</div>
<span class="con2">
<br>
</span>
<form id="form1" method="post" action="list"  class="form_search">
    ...
    柜台:<select name="counterId" class="ip" id="supplierIdSelect">
<#if counterList??>
    <#list counterList as s>
        <option value="${s.id?if_exists}" <#if s.id==counterId>selected</#if>>${s.name?if_exists}</option>
    </#list>
</#if>
</select>
...
<table class="bgg" bgcolor="#cccccc" border="0" cellpadding="0" cellspacing="0" width="100%">
    ...
    <#if list??>
        ...
        <tr onMouseOut="this.style.backgroundColor='ffffff'" onMouseOver="this.style.backgroundColor='FFECA2'" class="tr_display">
            ...
            <td align="center" bgcolor="#FFFFFF">
                <a href="toEditDtl?orderCounterId=${s.id?if_exists}" target="_self">操作</a>
            </td>
